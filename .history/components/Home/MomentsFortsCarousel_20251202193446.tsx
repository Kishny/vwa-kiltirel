...
function MomentCard({ label, tag }: { label: string; tag: string }) {
    return (
        <article className="h-full rounded-3xl bg-white border border-black/5 shadow-sm px-4 py-3 flex flex-col justify-between">
            <p className="text-sm font-medium text-vwa-dark">{label}</p>
            <span className="mt-3 inline-flex w-fit rounded-pill bg-vwa-primary/8 px-2.5 py-1 text-[11px] text-vwa-primary">
                {tag}
            </span>
        </article>
    );
...
export default function MomentsFortsCarousel() {
    ...
    return (
        <section className="mt-8">
            ...
            {/* Mobile : carrousel swipe horizontal */}
            <div
                ref={containerRef}
