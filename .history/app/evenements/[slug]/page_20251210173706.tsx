...
import { mediaByEvent } from "@/data/media"; // 🔥 nouveau import
import {
    CalendarDaysIcon,
    MapPinIcon,
    CurrencyEuroIcon,
...
export default async function EventDetailPage({
    ...
    // 🔗 Récup des médias liés à cet événement
    const eventMedias = mediaByEvent[event.slug] ?? [];
    const hasMedia = eventMedias.length > 0;
    const previewMedias = hasMedia ? eventMedias.slice(0, 3) : [];
                ...
                <div className="p-5 sm:p-7 space-y-8">
                        ...
                        <div className="relative">
                            ...
                            <div className="relative rounded-3xl bg-vwa-dark text-vwa-background px-5 py-4 shadow-[0_20px_60px_rgba(28,22,18,0.7)]">
                                ...
                                <p className="mt-2 text-sm">
                                    Un moment chaleureux, bienveillant et intimiste pour faire
                                    vibrer les cultures créoles, afro-descendantes et
                                    caribéennes, en famille ou entre ami·e·s.
                                </p>
                                {!isPast && (
                                    <p className="mt-3 text-[11px] text-vwa-background/75">
                                        Les places sont limitées : pense à confirmer ton
                                        inscription pour recevoir ton ticket ou ton QR code par
                                        e-mail.
                                    </p>
                                )}
                            </div>
                    ...
                    <section className="space-y-3">
                        ...
                        <ul className="list-disc list-inside space-y-1.5 text-sm text-vwa-dark/75">
                            ...
                            <li>
                                En cas d&apos;empêchement, merci de nous prévenir au moins
                                48&nbsp;h à l&apos;avance.
                            </li>
                            <li>
                                L&apos;adresse exacte et les infos pratiques sont rappelées
                                dans l&apos;e-mail de confirmation après inscription.
                            </li>
                            <li>
                                Selon l&apos;événement, un ticket ou QR code pourra être
                                demandé à l&apos;entrée.
                            </li>
                        </ul>
                    </section>

                    {/* ⭐ Souvenirs en images (mini preview) */}
                    {hasMedia && (
                        <section className="space-y-3 pt-4 border-t border-vwa-background/70">
                            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                <div className="space-y-1">
                                    <h2 className="text-xs font-semibold uppercase tracking-[0.18em] text-vwa-dark/60">
                                        Souvenirs en images
                                    </h2>
                                    <p className="text-xs text-vwa-dark/70 max-w-md">
                                        Quelques aperçus de cet événement. Retrouvez la galerie
                                        complète dans la médiathèque Vwa Kiltirèl.
                                    </p>
                                </div>
                                <Link
                                    href={`/mediatheque?event=${event.slug}`}
                                    className="inline-flex items-center justify-center gap-2 rounded-full bg-vwa-dark px-4 py-1.5 text-[11px] font-semibold text-vwa-background shadow-sm hover:bg-black hover:shadow-md transition-all"
                                >
                                    <PhotoIcon className="h-4 w-4" />
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                {previewMedias.map((media) => (
                                    <div
                                        key={media.id}
                                        className="group relative aspect-[4/3] overflow-hidden rounded-2xl bg-vwa-background/70 shadow-[0_14px_40px_rgba(28,22,18,0.12)]"
                                    >
                                        <Image
                                            src={media.src}
                                            alt={media.alt}
