...
export default function EventInscriptionPage({ params }: EventInscriptionPageProps) {
    ...
    return (
        <main className="max-w-3xl mx-auto px-4 py-10 space-y-8">
            ...
            {/* Bloc infos tarif / réservation */}
            {!isPast && (
                <section className="space-y-3">
                    <div className="rounded-2xl border border-vwa-background bg-vwa-background/40 px-4 py-3 text-sm text-vwa-dark/85">
                        <p className="font-semibold">Tarifs & réservation</p>
                        ...
                        {event.paymentNote && (
                            <p className="mt-1">{event.paymentNote}</p>
                        )}

                        {event.reservationNote && (
                            <p className="mt-1">{event.reservationNote}</p>
                        )}

                        {event.ticketNote && (
                            <p className="mt-1">{event.ticketNote}</p>
                        )}
                    </div>
                    ...
                    {event.paymentUrl && (
                        <a
                            ...
                            className="inline-flex items-center justify-center rounded-full bg-vwa-accent px-5 py-2 text-sm font-medium text-white shadow-sm hover:bg-vwa-dark hover:shadow-md transition-all"
                        >
                            Accéder au paiement sécurisé (HelloAsso)
                        </a>
                    )}
                </section>
            )}
