...
export default function MediathequeView({
    ...
    // Si on change de catégorie et que l’événement actif ne matche plus,
    // on prend le premier du filtre
    useEffect(() => {
    ...
    // -----------------------
    const goToRelativeMedia = (delta: 1 | -1) => {
        ...
        const list =
            lightboxMedia.origin === "event" ? eventList : assoList;
    ...
    }, [lightboxMedia, activeMedias.length, associationMedia.length]);
        ...
        <div className="space-y-10">
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                    <div className="space-y-2">
                        ...
                        <p className="text-sm text-vwa-dark/75 max-w-2xl">
                            ...
                            événements Vwa Kiltirèl, ainsi que des instantanés de la vie
                            de l’association.
                        </p>
                    </div>
                    <div className="text-xs text-right space-y-1">
                        ...
                        <p className="text-[11px] text-vwa-dark/55">
                            ...
                            {associationMedia.length > 1 && "s"} “Vie de l’association”
                        </p>
                    </div>
                </div>
            </header>
            ...
            <section className="space-y-6">
                ...
                <section className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {categories.map((cat) => {
                            ...
                            const label =
                                cat === ALL_CATEGORIES ? "Tous les événements" : cat;
                            ...
                            return (
                                <button
                                    ...
                                    className={[
                                        "inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-medium transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-vwa-accent/70 focus-visible:ring-offset-2 focus-visible:ring-offset-vwa-background",
                                        isActive
                ...
                <section className="grid gap-6 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.4fr)] items-start">
                                    ...
                                    return (
                                        <li key={event.slug}>
                                            <button
                                                ...
                                                className={[
                                                    "w-full text-left px-4 py-3.5 transition-all duration-200 flex flex-col gap-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-vwa-accent/70 focus-visible:ring-offset-2 focus-visible:ring-offset-vwa-background",
                                                    isActive
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center justify-between text-[11px] text-vwa-dark/55">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
                                {activeMedias.map((media, index) => (
                                    <button
                                        ...
                                        className="group relative aspect-[4/3] overflow-hidden rounded-2xl bg-vwa-background/60 shadow-[0_14px_40px_rgba(28,22,18,0.12)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_22px_65px_rgba(28,22,18,0.22)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-vwa-accent/70 focus-visible:ring-offset-2 focus-visible:ring-offset-vwa-background"
                                    >
                                        <Image
                                            src={media.src}
                                            alt={media.alt}
                ...
                <section className="space-y-4 pt-4 border-t border-vwa-background/60">
                    <div className="flex items-baseline justify-between gap-4">
                        ...
                        <span className="text-[11px] text-vwa-dark/55">
                            {associationMedia.length} média
                            {associationMedia.length > 1 && "s"} (coulisses,
                            préparation, etc.)
                        </span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
                        {associationMedia.map((media, index) => (
                            <button
                                ...
                                className="group relative aspect-[4/3] overflow-hidden rounded-2xl bg-vwa-background/70 shadow-[0_12px_36px_rgba(28,22,18,0.16)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_20px_60px_rgba(28,22,18,0.24)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-vwa-accent/70 focus-visible:ring-offset-2 focus-visible:ring-offset-vwa-background"
                            >
                                <Image
                                    src={media.src}
                                    alt={media.alt}
                ...
                >
                    ...
                    >
                        ...
                        <button
                            type="button"
                            onClick={handlePrev}
                            className="absolute left-3 top-1/2 -translate-y-1/2 inline-flex h-9 w-9 items-center justify-center rounded-full bg-black/70 text-white/90 text-lg shadow-lg hover:bg-black focus:outline-none"
                        >
                            ‹
                        </button>
                        <button
                            type="button"
                            onClick={handleNext}
                            className="absolute right-3 top-1/2 -translate-y-1/2 inline-flex h-9 w-9 items-center justify-center rounded-full bg-black/70 text-white/90 text-lg shadow-lg hover:bg-black focus:outline-none"
                        >
                            ›
                        </button>
